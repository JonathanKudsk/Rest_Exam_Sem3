...
public class Utils {
    ...
    public static String getPropertyValue(String propName, String resourceName)  {
        ...
        try (InputStream is = Utils.class.getClassLoader().getResourceAsStream(resourceName)) {
            ...
            if (value != null) {
                return value.trim();  // Fjern whitespace
            } else {
    ...
    public ObjectMapper getObjectMapper() {
        ...
        objectMapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false); // Ignorer ukendte properties i JSON
    ...
    public static String convertToJsonMessage(Context ctx, String property, String message) {
        ...
        msgMap.put(property, message);  // Tilføj beskeden til map
        msgMap.put("status", String.valueOf(ctx.status()));  // Tilføj status til map
        ObjectMapper objectMapper = new ObjectMapper();
        try {
            return objectMapper.writeValueAsString(msgMap);  // Konverter map til JSON
        } catch (Exception e) {
