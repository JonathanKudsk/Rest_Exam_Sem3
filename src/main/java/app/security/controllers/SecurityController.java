...
public class SecurityController implements ISecurityController {
    ...
    // Singleton pattern - vi vil ikke have flere instanser af samme klasse
    ...
    public Handler register() {
        return (ctx) -> {
            ObjectNode returnObject = objectMapper.createObjectNode();
            try {
                UserDTO userInput = ctx.bodyValidator(UserDTO.class)
                        .check(u -> u.getUsername() != null && !u.getUsername().isEmpty(), "User is required")
                        .check(u -> u.getPassword() != null && !u.getPassword().isEmpty(), "Password is required")
                        .get();
    ...
    public Handler authenticate() throws UnauthorizedResponse {
        ...
        ObjectNode returnObject = objectMapper.createObjectNode();
        return (ctx) -> {
            // Dette er en preflight request => OK
    ...
    @Override
    // Tjek om brugerens roller indeholder nogen af de tilladte roller
    public boolean authorize(UserDTO user, Set<RouteRole> allowedRoles) {
        if (user == null) {
            throw new UnauthorizedResponse("User session not found. Please log in to continue.");
        }
        Set<String> roleNames = allowedRoles.stream()
                   .map(RouteRole::toString)  // Konverter RouteRoles til Set af Strings
    ...
    // Health check for API. Bruges i deployment
